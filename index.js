class GameRedpacket {
  ...
  init() {
    ...
    clearTimeout(this.timeout_gameTime);
  }
  // 开始游戏
  startGame() {
    this.init();
  ...
  getRedPacketObjById(id) {
    const INDEX = this.redPacketList
      .map(o => {
        return o.id;
      })
      .indexOf(id);
...
class RedPacket {
  ...
  initStart() {
    ...
    this.speed = Math.random() * 2 + 3;
    this.x =
      (this.gameRedpacket.redPacketWarp.width - this.width) * Math.random();
